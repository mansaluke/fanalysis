...
def append_df(att1,att2) -> DataFrame:
      ...
      def do_append_after(fn):
            ...
            def append_after(self, *args, **kwargs):
                ...
                for col in cols_r:
                    returned_df = returned_df.rename({col:change_colname(col, cols_o, cols_r)}, axis=1) 

                appended_df = original_df.merge(returned_df, on = datename, how = 'left')
...
class append_data(create_data):
      ...
      def __init__(self, df):
            self.df = df
            self.datename = self.df.select_dtypes(include=[np.datetime64]).columns[0]
...
if __name__ == '__main__':
    ...
    df = create_data('d', date = datetime.now(), direction='Backwards').create_brownian_motion()
